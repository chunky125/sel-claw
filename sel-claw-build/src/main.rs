...
use sel_claw_build::model::{Arch, Platform, SeL4Arch};
...
fn build_kernel(
    ...
    SeL4BuildOutcome,
    sel_claw_build::model::contextualized::Contextualized,
) {
    ...
    let config = sel_claw_build::model::contextualized::Contextualized::from_str(
        &config_content,
        build_params
            .arch
...
mod simulate {
    ...
    use sel_claw_build::model::{SeL4Arch, SingleValue};
    use std::path::{Path, PathBuf};
    use std::process::{Command, Stdio};
